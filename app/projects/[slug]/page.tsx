...
export default function ProjectPage({ params }: ProjectPageProps) {
  ...
  const getImageSrc = (imagePath: string) => {
    if (imageError) {
      return "/placeholder.svg?height=400&width=800&text=Project+Cover"
    }
    return imagePath || "/placeholder.svg?height=400&width=800&text=Project+Cover"
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="relative z-10 container mx-auto p-3 sm:p-4 pt-20 sm:pt-24 pb-6 sm:pb-8">
          ...
          <AnimatedSection animation="fade-up" className="lg:col-span-3">
            <Card className="bg-zinc-900/70 border-zinc-800 backdrop-blur-sm overflow-hidden">
              <div className="relative h-48 sm:h-64 md:h-80 w-full">
                <Image
                  src={getImageSrc(project.coverImage) || "/placeholder.svg"}
                  alt={project.title}
                  fill
          ...
          <div className="space-y-4 sm:space-y-6">
            {relatedProjects.length > 0 && (
              <AnimatedSection animation="slide-left" delay={200}>
                <Card className="bg-zinc-900/70 border-zinc-800 backdrop-blur-sm">
                  <CardContent className="p-4 sm:p-6">
                    <h2 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4">All Projects</h2>
                            ...
                            <div className="flex items-center gap-2 sm:gap-3">
                              <div className="relative w-12 h-12 sm:w-16 sm:h-16 rounded overflow-hidden flex-shrink-0">
                                <Image
                                  ...
                                  unoptimized
                                  onError={(e) => {
                                    e.currentTarget.src = "/placeholder.svg?height=64&width=64&text=Project"
                                  }}
                                />
                              </div>
                              <div>
        ...
        >
          <p>© {new Date().getFullYear()} Pranavh Harish Sivakumar. All rights reserved.</p>
        </AnimatedSection>
      </div>
